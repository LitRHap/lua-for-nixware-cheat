...
local lua_ah_alternative_desync = ui.add_combo_box("Alternative desync", "lua_ah_alternative_desync", { "None", "Anti bruteforce", "LowDelta" }, 0)
...
local function get_manual_side() -- drunk coding owns me and all
	if not manual_hold then
		if lua_ah_manual_left:is_active() and manual_side ~= 1 then
			manual_hold = true
			manual_side = 1
		elseif lua_ah_manual_back:is_active() and manual_side ~= 2 then
			manual_hold = true
			manual_side = 2
		elseif lua_ah_manual_right:is_active() and manual_side ~= 3 then
			manual_hold = true
			manual_side = 3
		end
	...
	if not lua_ah_manual_left:is_active() and not lua_ah_manual_back:is_active() and not lua_ah_manual_right:is_active() then
		manual_hold = false
	end
	return manual_side
end
...
client.register_callback("bullet_impact", function(event)
	...
	if dist < 45 and dist_3d(from, to) >= dist_3d(from, head) and player_shots[engine.get_player_for_user_id(event:get_int("userid", 0))] + 0.25 < globalvars.get_current_time() then
...
local function on_create_move(cmd)
	...
	if cmd.send_packet then
		...
		if lua_ah_desync_jitter:get_value() then
			desync_flipped = not desync_flipped
		end
	end
