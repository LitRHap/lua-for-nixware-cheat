local rpg_ball_texture = renderer.setup_texture("nix\\images\\avatar.jpg")

local screensize = engine.get_screen_size()

local rpg_ball_on  = ui.add_check_box("Enable Anime Ball", "rpg_ball_on", false)

local rpg_ballx = ui.add_slider_int("Anime Ball POS X", "rpg_ball_x", 0, screensize.x, 10)

local rpg_bally = ui.add_slider_int("Anime Ball POS Y", "rpg_ball_y", 0, screensize.y, 10)

local m_iHealth = se.get_netvar("DT_BasePlayer", "m_iHealth")

local Verdana_bold = renderer.setup_font("C:/windows/fonts/verdana.ttf", 14, 0)
...
local function rpg_paint()
    ...
    rect_filled_fade(rpg_ballx:get_value() + 222,rpg_bally:get_value() + 5,20,20,color_t.new(45,45,45,255),color_t.new(45,45,45,0),color_t.new(45,45,45,0),color_t.new(45,45,45,255))

    rect_filled_fade(rpg_ballx:get_value() + 92,rpg_bally:get_value() + 5,130,20,color_t.new(45,45,45,255),color_t.new(45,45,45,255),color_t.new(45,45,45,255),color_t.new(45,45,45,255))
    ...
    text(rpg_ballx:get_value() - 67,rpg_bally:get_value() + 33,heal,13,color_t.new(250 - health * 2.5, 5 + health * 2.5, 0, 255))

    text(rpg_ballx:get_value() + 104,rpg_bally:get_value() + 8,username .. " | Wlecome home",12,color_t.new(255,255,255, 255))

    text(rpg_ballx:get_value() + 103,rpg_bally:get_value() + 58,"Just Crow",12,color_t.new(255,255,255, 255))
