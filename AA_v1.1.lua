require("bit32")
local choke_int_slider = ui.add_slider_int("Choke", "choke_int", 1, 11, 1)
local send_int_slider = ui.add_slider_int("Send", "send_int", 1, 11, 1)
...
local e_legit_checkbox = ui.add_check_box("Legit aa on E", "e_legit_aa", false)
...
local function aa(pCmd)
    ...
    if lowdelta_sw_checkbox:get_value() then
        ...
        desync_delta = sw_bind and 100 or des1*side
    else    
        desync_delta = des1 * side
    end  
            
    pCmd.viewangles.pitch = pitch1
    pCmd.viewangles.yaw = pCmd.viewangles.yaw + adding

    if not pCmd.send_packet then
        pCmd.viewangles.yaw = pCmd.viewangles.yaw + desync_delta
    else
        pCmd.viewangles.yaw = pCmd.viewangles.yaw + yaw1
    end    
end
